...
class contrail::analyticsdatabase::config (
  ...
  $settings_vtokens = deep_merge($settings_no_vtokens, {
    'num_tokens'  => 256,
  })
  $check_num_tokens_cmd = "grep -v '#' '${::cassandra::params::config_path}/cassandra.yaml' | head -n 1)  | grep -q 'num_tokens'"
  ...
  file { ['/var/lib/cassandra', ]:
    ...
    group   => 'cassandra',
    mode    => '0755',
    before  => Class['cassandra_vtokens', 'cassandra_no_vtokens'],
  }

  class { 'cassandra_vtokens':
    package_name    => 'cassandra',
    hints_directory => '/var/lib/cassandra/hints',
    settings        => $settings_vtokens,
    onlyif          => $check_num_tokens_cmd,
  }
  class { 'cassandra_no_vtokens':
    package_name    => 'cassandra',
    hints_directory => '/var/lib/cassandra/hints',
    settings        => $settings_no_vtokens,
    unless          => $check_num_tokens_cmd,
  }
